...
function renderAdmin(){
  ...
  visible.forEach(s => {
    ...
    ac.innerHTML = `<div class="admin-line"><strong>${s.type} • Station ${s.station_number}</strong><span class="status">${statusLabel(s)}${rem ? ` • ${fmt(rem)}` : ""}</span></div><div class="admin-controls"><button class="${s.status === "available" ? "active" : ""}" onclick="setStatus(${s.id},'available')">Tersedia</button><button class="${s.status === "occupied" ? "active" : ""}" onclick="setStatus(${s.id},'occupied')">Terisi</button><button class="${s.status === "offline" ? "active" : ""}" onclick="setStatus(${s.id},'offline')">Tidak Aktif</button></div><div class="admin-time"><input id="time-${s.id}" type="number" min="30" step="30" max="9999" placeholder="Durasi (menit)" oninput="previewEndTime(${s.id})"><button onclick="setMinutes(${s.id})">Simpan</button></div><div class="admin-endtime" id="endtime-${s.id}">Estimasi selesai: <strong>${endClock}</strong></div>`;
