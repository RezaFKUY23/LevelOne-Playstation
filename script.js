...
async function renderRevenueChart(dateStr, period){
  ...
  else { const days=new Date(start.getFullYear(),start.getMonth()+1,0).getDate(); for(let i=0;i<days;i++) buckets.push({label:String(i+1),value:0}); }
  rows.forEach(r=>{const d=new Date(r.started_at); let idx=period==='day'?d.getHours():period==='week'?Math.floor((d-start)/86400000):d.getDate()-1;if(buckets[idx]) buckets[idx].value += Number(r.amount||0);});
